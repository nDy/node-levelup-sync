...
LevelUP.prototype.open = function (callback) {
  ...
  if (callback) db.open(this.options, function (err) {
    if (err) {
...
LevelUP.prototype.isExists = function (key_, options, callback) {
  var self = this
    , key
  ...
  if (key_ === null || key_ === undefined)
    return readError(this
      , 'isExists() requires key arguments', callback)

  options = util.getOptions(this, options)
  key = this._codec.encodeKey(key_, options)

  if (callback) {
    this.db.isExists(key, options, function (err, value) {
      if (err) {
  ...
  } else return this.db.isExists(key, options)
}
...
LevelUP.prototype.get = function (key_, options, callback) {
  ...
  if (key_ === null || key_ === undefined)
    return readError(this
      , 'get() requires key arguments', callback)
  ...
  } else return this.db.get(key, options)
}
...
LevelUP.prototype.put = function (key_, value_, options, callback) {
  ...
  if (callback) this.db.put(key, value, options, function (err) {
    if (err) {
  ...
  else return this.db.put(key, value, options)
}
...
LevelUP.prototype.del = function (key_, options, callback) {
  ...
  if (callback) this.db.del(key, options, function (err) {
    if (err) {
      return dispatchError(self, new WriteError(err), callback)
    } else {
      self.emit('del', key_)
  ...
  else return this.db.del(key, options)
}
...
LevelUP.prototype.batch = function (arr_, options, callback) {
  ...
  if (callback) this.db.batch(arr, options, function (err) {
    if (err) {
      return dispatchError(self, new WriteError(err), callback)
    } else {
  ...
  else return this.db.batch(arr, options)
}
